...
class OutlineGenerator:
    ...
    OUTLINE_TEMPLATES = {
        ...
        "逃亡_追击": ChapterOutline(
            chapter_number=1,
            title="绝境奔逃",
            scenes=["发现追踪", "利用环境", "险象环生"],
            key_events=["暴露行踪", "设置障碍", "摆脱追击"],
            emotional_arc="惊恐→冷静→决断→喘息"
        ),
        "背叛_揭露": ChapterOutline(
            chapter_number=1,
            title="面具之下",
            scenes=["发现端倪", "证据确凿", "对峙摊牌"],
            key_events=["发现异常", "真相浮现", "关系破裂"],
            emotional_arc="信任→怀疑→愤怒→清醒"
        ),
        "告别_离别": ChapterOutline(
            chapter_number=1,
            title="最后一面",
            scenes=["倒计时", "未说之话", "转身离开"],
            key_events=["分别在即", "克制情感", "各自上路"],
            emotional_arc="不舍→压抑→释然→思念"
        ),
    }
...
class EnhancedImitationLearning:
    ...
    def generate_training_data(
        self,
        num_samples: int = 432,  # V2: 12场景 × 6变体 × 2(结构化+简化) = 432 (原120)
        output_length: int = 400,  # V2: 增加到400字/条
        ...
        # 定义场景类型和对应的prompt模板（V2：12场景覆盖更广）
        scene_configs = [
            # ── 紧张/悬疑类 ──
            {
                "type": "废土列车_发现",
            ...
            },
            {
                ...
                "requirements": ["紧张感", "动作", "悬念"],
            },
            {
                "type": "逃亡_追击",
                "description": "描写主角在被追捕过程中利用环境和智慧摆脱危险的场景，节奏紧凑。",
                "requirements": ["紧张感", "动作", "节奏"],
            },
            # ── 对白/冲突类 ──
            {
                "type": "资源_冲突",
            ...
            {
                "type": "背叛_揭露",
                "description": "写一段主角发现最信任的人背叛了自己，双方对峙摊牌的场景，情感张力要足。",
                "requirements": ["对白", "情感", "冲突"],
            },
            # ── 内心/情感类 ──
            {
                "type": "深夜_回忆",
            ...
            {
                "type": "告别_离别",
                "description": "写一段重要角色之间生离死别的场景，克制中蕴含深情，不要过度煽情。",
                "requirements": ["情感", "描写"],
            },
            # ── 探索/发现类 ──
            {
            ...
            {
                "type": "反转_真相",
                "description": "写一段主角发现自己一直以来的认知完全错误的场景，通过细节层层揭示真相。",
                "requirements": ["悬念", "逻辑", "节奏"],
            },
        ]
    ...
    def _generate_previous_summary(self, scene_type: str, variation: int) -> str:
        ...
        summaries = {
            ...
            # ── V2 新增场景 ──
            "逃亡_追击": [
                "身后传来密集的脚步声和低沉的咆哮。李明知道，它们追上来了。他咬紧牙关，冲进前方错综复杂的废弃巷道——这是他唯一的机会。",
                "警报声在整个基地回荡。陈风抓起背包就往外跑，身后传来防爆门被撞开的轰响。三条街道，只有一条通向安全区。",
                "水流的冲击力差点把林雪卷入深渊。她死死抱住一块凸起的岩石，身后是追踪者手电筒的光束在河面上扫来扫去。",
            ],
            "背叛_揭露": [
                "那封加密邮件的发送者IP地址，和李明最信任的人完全一致。他的手开始颤抖，不敢相信眼前的事实。",
                "保险柜里的文件不翼而飞。唯一有权限的人，只有三个人——而其中一个人刚刚还在说会帮他到底。",
                "\"是你告诉他们的，对吗？\" 李明的声音出奇地平静。对面的人沉默了三秒，那三秒比一辈子还长。",
            ],
            "告别_离别": [
                "列车还有五分钟发车。他们站在站台上，中间隔着一段再也跨不过去的距离。千言万语，最后只成了一句：\"保重。\"",
                "医院的仪器发出长鸣。她握住那只逐渐变凉的手，想起第一次见面时对方说的那句\"你好\"。原来有些告别，永远没有准备好的那一天。",
                "\"等我回来。\" 他翻身上马，头也不回地冲进漫天风沙中。她站在城楼上，直到那个身影消失在地平线，才允许自己掉下第一滴眼泪。",
            ],
            "反转_真相": [
                "档案编号7049的最后一行写着：实验体存活。李明感觉天旋地转——这三年来，他一直在寻找的自己，其实就是被官方宣告死亡的...自己。",
                "所有证据都指向同一个人，而那个人已经在半年前\"死\"了。陈风重新梳理时间线，发现了一个令人毛骨悚然的规律：每一起案件的受益者，都是同一个名字。",
                "她以为自己在救赎一个迷途的灵魂，直到那天在他书房里发现了那本日记。原来从第一天起，猎人装扮成了猎物，而她才是那个真正被困在网里的人。",
            ],
        }
...
李明猛地拐进一条狭窄的巷子，肩膀蹭掉了一层墙皮。他知道这条路——七年前在这片废墟上讨生活时，每一条巷道他都烂熟于心。

前方三米处是一堵塌了一半的墙。他没有减速，而是整个人贴地滑了过去，在碎石地上留出一道长长的划痕。

身后传来追捕者的嘶吼声。它们闻到了他的气味。

李明从墙的另一侧翻起来，发现面前是一个死胡同。

不——不是死胡同。角落里有一个生锈的通风口，勉强能容一个人爬过去。他二话不说，把手伸进去用力一扯——锈死的栅栏应声而断。

就在他钻进去的瞬间，一只长满鳞片的手从后面抓住了他的脚踝。

"放开！" 李明抽出腰间的匕首，反手就是一刀。

那东西缩回了手。李明趁机钻进通风口，用背死死顶住栅栏。

黑暗中，他和那个怪物只隔着一层铁条。双方都能听到彼此的呼吸声。""",
                """水流的速度比想象中快得多。林雪被卷入漩涡的瞬间，脑子里只有一个念头：不能松手。

她的指甲劈进了岩石缝隙，指节因为过度用力而发白。河水像无数只冰凉的手，拽着她的腿往下拉。

岸上，手电筒的光束扫过河面。

"她往那边去了！"

林雪把脸埋进水里，只留鼻子和嘴巴露出水面换气。冰冷的河水灌进耳朵，嗡嗡作响。

光束在她上方停留了三秒，然后移开了。

她没有动。又过了十秒，她才开始慢慢地、一点一点地把身体往岸边挪。每一厘米都像是用尽了她毕生的力气。

终于，她的手碰到了浅水区的鹅卵石。""",
            ],
            "背叛_揭露": [
                """屏幕上的IP地址像一根针，扎穿了李明三个月来的信任。

发送者：192.168.1.17

他记得这个地址。这是老张家的IP——那个在他最困难的时候收留他、教他生存技巧、在无数次危险中挡在他身前的男人。

邮件的内容更简单：

"目标已按计划进入区域。一切准备就绪。随时可以行动。"

发送时间：三天前。

三天前——正是老张主动提出要带他去"寻找补给"的那天。那天他差点回不来。

门响了一下。

李明没有回头。他的手放在桌上，距离那把匕首只有五厘米。

"在看什么呢？" 老张的声音和平时一样温和。

但李明现在听出了不一样的东西——那是猎人在看走进陷阱的猎物时的语气。""",
                """保险柜里的文件不见了。

不是撬开的——锁完好无损，没有暴力破坏的痕迹。这意味着有密码的人拿走了它。

知道密码的三个人：队长、苏婉、还有陈锋自己。

队长三天前出外勤，至今未归。苏婉正在医疗室值班，有十二个目击证人。

陈锋站在保险柜前，慢慢关上了柜门。

他想起昨天吃饭时，苏婉多看了他一眼。当时他没在意。现在回想起来，那个眼神里有一种他说不清的东西——不是关心，不是好奇，而是某种接近于...确认的东西。

口袋里的手机震动了。一条来自陌生号码的信息：
...
"真的不走？" 她的声音很轻，轻得几乎被广播盖过。

"任务还没完成。" 他说，声音比他想的不稳得多，"等我回来——"

"上次你也这么说。"

沉默。

上一次，他也是这样走的。那时候她说好等他，可等到的是一封冰冷的信和一个已经搬空的公寓。

但这次不同。这一次，他们都清楚这可能真的是最后一次见面了。

"听着，" 他突然伸手抱住了她，"不管发生什么——"

"别。" 她推开了他，眼眶发红却没让眼泪掉下来，"别说什么'无论如何'。你就好好活着。这比什么都强。"

列车员开始关闭车门。

他转身走上了车厢。没有回头。因为他知道如果回头一次，他就再也走不了了。

车窗缓缓升起的时候，他终于忍不住转过头——站台上空无一人。
...
这不是一份普通的档案。它被封存在系统最深处，访问权限为最高级别——整个基地只有三个人能打开。

而现在，李明就坐在终端前，看着屏幕上一行行让他血液冻结的文字：

**实验体编号：E-7049**
**状态：存活（确认于2089年3月15日）**
**当前位置：未知**
**备注：该实验体对自身身份完全无知。建议维持现状。**

李明的手在颤抖。

他看向终端边缘倒映出的那张脸——自己的脸。然后他低头看了看左腕内侧，那里有一串从小到大就存在的数字编码：E-7049。

三年来，他一直在寻找一个叫 E-7049 的实验体。组织告诉他，那个人是唯一的希望，是唯一可能拯救人类的存在。

他从没想到，他要找的人，就是他自己。

而那些一直在"帮助"他的人，不过是把他当成了实验品在观察。
...
"你看得比预期早了一些。" 一个熟悉的声音说。""",
    ...
    def _get_simple_prompt_for_context(self, ctx: WritingContext) -> str:
        ...
        scene_to_prompt = {
            ...
            # V2 新增
            "逃亡_追击": "描写主角在被追捕过程中利用环境和智慧摆脱危险的场景，节奏紧凑。",
            "背叛_揭露": "写一段主角发现最信任的人背叛了自己，双方对峙摊牌的场景，情感张力要足。",
            "告别_离别": "写一段重要角色之间生离死别的场景，克制中蕴含深情。",
            "反转_真相": "写一段主角发现自己一直以来的认知完全错误的场景，层层揭示真相。",
        }
