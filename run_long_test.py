...
def poll_task(task_id, step_name, max_attempts=180, interval=10):
    """轮询任务 - 最长30分钟"""
    ...
    log('\n[TIMEOUT] Polling timeout after 30 minutes')
    return False
...
def write_chapter():
    """章节写作"""
    log("\n" + "="*70)
    log("CHAPTER WRITING (Writer-Reader Loop)")
    log("="*70)
    ...
    payload = {
        'chapter_number': 1,
        'writing_style': 'Narrative fluent with vivid descriptions',
        'env_description_level': 'normal',
        'dialogue_ratio': 0.3,
        'notes': 'Real Writer-Reader loop test with 30min timeout',
        'outline': outline,
        'auto_mode': False,
        'timeout': TIMEOUT
    }
...
def main():
    log("="*70)
    log("REAL LLM WRITING TEST - 30min timeout")
    log("="*70)
